...
@section('content')
    <form action="{{ url("/students/$student->nim") }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-12 items-center">
            ...
            <input type="text" id="studyProgram" name="studyProgram" value="{{ $student->studyProgram }}"
                class="col-span-10 mt-1 p-1.5 border rounded">
        </div>
        ...
        <div class="grid grid-cols-12 items-center">
            ...
            <select id="faculty" name="faculty" class="col-span-10 mt-1 p-1.5 border rounded">
                ...
                <option value="Kedokteran" {{ $student->faculty == 'Kedokteran' ? 'selected' : '' }}>Kedokteran</option>
                <option value="MIPA" {{ $student->faculty == 'MIPA' ? 'selected' : '' }}>MIPA</option>
                <option value="Teknik" {{ $student->faculty == 'Teknik' ? 'selected' : '' }}>Teknik</option>
            </select>
        </div>
